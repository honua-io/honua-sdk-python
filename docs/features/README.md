...
- `honua-gp` proprietary drop-in shim covering 45 top-of-corpus `arcpy` functions (15 analysis + 20 management + 10 data-access); dispatches through the existing `honua-sdk` / `honua-admin` / OGC API Processes clients and writes a redacted JSONL audit per call.
...
- ArcPy compatibility shim: `packages/honua-gp/honua_gp/`
- Examples: `examples/`
- Tests: `tests/`
...
The Python SDK is alpha. Keep docs explicit about current read/write smoke coverage, optional dependencies, and server compatibility gates before making stable API claims. `honua-gp` is proprietary and excluded from the public Apache-2.0 grant; see [docs/honua-gp/](../honua-gp/README.md).
