"""Pytest fixtures for the honua-gp test suite."""
...
# Ensure the sibling honua-sdk / honua-admin / honua-gp packages are on sys.path
...
for relative in ("packages/honua-sdk", "packages/honua-admin", "packages/honua-gp"):
...
def _isolated_audit_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Iterator[Path]:
    ...
    monkeypatch.setenv("HONUA_GP_AUDIT_DIR", str(audit_dir))
    yield audit_dir
