...
:class:`~honua_gp._errors.HonuaGpUnsupportedError` with a tracking
ticket. They are listed in the compatibility manifest so the matrix and
``honua-gp assess`` show them.
"""
...
def _payload_for_row(row: Sequence[Any], fields: Sequence[str]) -> dict[str, Any]:
    ...
    # HonuaGpConfigurationError (which subclasses ExecuteError) instead of
...
def _unsupported_search_cursor_option(option: str, detail: str) -> None:
    raise HonuaGpUnsupportedError(
        "da.SearchCursor",
        compat_anchor=anchor_for("da.SearchCursor"),
        replacement_hint=f"{option} is not supported by honua-gp SearchCursor. {detail}",
    )
...
class _BaseCursor:
    ...
    def __enter__(self) -> "_BaseCursor":
        ...
        try:
            ...
            # (e.g. HonuaGpConfigurationError) instead of a stray
...
class UpdateCursor(_BaseCursor):
    ...
    def updateRow(self, row: Sequence[Any]) -> None:
        ...
        if self._current_feature is None:
            raise HonuaGpConfigurationError("updateRow called before the cursor produced a row.")
        payload = _payload_for_row(row, self.fields)
        oid = self._extract_oid(self._current_feature)
        if oid is None:
            raise HonuaGpConfigurationError("updateRow requires an OID-bearing row; include OID@ in field_names.")
    ...
    def deleteRow(self) -> None:
        self._ensure_open()
        if self._current_feature is None:
            raise HonuaGpConfigurationError("deleteRow called before the cursor produced a row.")
        oid = self._extract_oid(self._current_feature)
        if oid is None:
            raise HonuaGpConfigurationError("deleteRow requires an OID-bearing row; include OID@ in field_names.")
    ...
    def flush(self) -> dict[str, Any] | None:
        ...
        if not hasattr(self._source, "apply_edits"):
            raise HonuaGpConfigurationError("Configured source does not expose apply_edits.")
        kwargs: dict[str, Any] = {}
        if self._updates:
