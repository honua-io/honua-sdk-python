...
def test_search_cursor_with_where_clause(stub_clients) -> None:
    with honua_gp.da.SearchCursor("roads", ["OID@", "STATUS"], where_clause="STATUS = 'OPEN'") as cursor:
        rows = list(cursor)
    assert rows
...
def test_search_cursor_inherits_alias_where_from_make_feature_layer() -> None:
    ...
    with honua_gp.da.SearchCursor("roads_lyr", ["OID@", "STATUS"]) as cursor:
        list(cursor)
...
def test_search_cursor_uses_alias_workspace_after_env_workspace_changes() -> None:
    ...
    honua_gp.configure(client=_RecordingClient())
    honua_gp.env.workspace = "honua://services/original"
    honua_gp.management.MakeFeatureLayer("roads", "roads_lyr")
    honua_gp.env.workspace = "honua://services/changed"

    with honua_gp.da.SearchCursor("roads_lyr", ["OID@"]) as cursor:
        list(cursor)
...
def test_search_cursor_combines_alias_where_with_cursor_where() -> None:
    ...
    with honua_gp.da.SearchCursor("roads_lyr", ["OID@"], where_clause="OBJECTID > 0") as cursor:
        list(cursor)
...
def test_search_cursor_rejects_unsupported_options() -> None:
    ...
    with pytest.raises(honua_gp.HonuaGpUnsupportedError):
        with honua_gp.da.SearchCursor("roads", ["OID@"], explode_to_points=True) as cursor:
            list(cursor)

    with pytest.raises(honua_gp.HonuaGpUnsupportedError):
        with honua_gp.da.SearchCursor("roads", ["OID@"], sql_clause=("DISTINCT", None)) as cursor:
            list(cursor)
...
def test_update_cursor_inherits_alias_where() -> None:
    ...
    with honua_gp.da.UpdateCursor("roads_lyr", ["OID@", "STATUS"]) as cursor:
        list(cursor)
...
def test_insert_cursor_records_inserts(stub_clients) -> None:
    with honua_gp.da.InsertCursor("roads", ["STATUS", "name"]) as cursor:
...
def test_cursor_must_be_used_as_context_manager(stub_clients) -> None:
    cursor = honua_gp.da.SearchCursor("roads", ["OID@"])
...
def test_da_stubs_raise(stub_clients) -> None:
    with pytest.raises(honua_gp.HonuaGpUnsupportedError):
        honua_gp.da.Walk("honua://services/transport")
    with pytest.raises(honua_gp.HonuaGpUnsupportedError):
        honua_gp.da.TableToNumPyArray("segments", ["OBJECTID"])
...
def test_search_cursor_wraps_iter_features_failures_in_execute_error() -> None:
    ...
    with pytest.raises(honua_gp.ExecuteError) as info:
        with honua_gp.da.SearchCursor("roads", ["OID@"]) as cursor:
            next(iter(cursor))
...
def test_insert_cursor_flush_failure_wraps_in_execute_error_and_audits_error() -> None:
    ...
    with pytest.raises(honua_gp.ExecuteError) as info:
        with honua_gp.da.InsertCursor("roads", ["STATUS"]) as cursor:
...
def test_search_cursor_preserves_zero_valued_objectid() -> None:
    ...
    honua_gp.configure(client=_ZeroOidClient())

    with honua_gp.da.SearchCursor("roads", ["OID@", "STATUS"]) as cursor:
        rows = list(cursor)
...
def test_search_cursor_preserves_zero_valued_fid_fallback() -> None:
    ...
    honua_gp.configure(client=_FidClient())

    with honua_gp.da.SearchCursor("parcels", ["OID@", "NAME"]) as cursor:
        rows = list(cursor)
...
def test_update_cursor_preserves_zero_valued_oid_for_updates_and_deletes() -> None:
    ...
    methods raised HonuaGpConfigurationError, masking a valid edit.
    """
...
def test_cursor_open_failure_reports_real_error_kind(tmp_path) -> None:
    ...
    honua_gp.reset()
    with pytest.raises(honua_gp.HonuaGpConfigurationError):
        with honua_gp.da.SearchCursor("roads", ["OID@"]):
            pass
    ...
    # The error_kind attribute on HonuaGpConfigurationError is "configuration".
...
def test_update_cursor_next_before_enter_raises_configuration_error() -> None:
    cursor = honua_gp.da.UpdateCursor("roads", ["OID@", "STATUS"])
    with pytest.raises(honua_gp.HonuaGpConfigurationError):
        next(cursor)
...
def test_insert_cursor_reset_does_not_clear_buffer_before_raising() -> None:
    ...
    with honua_gp.da.InsertCursor("roads", ["STATUS"]) as cursor:
        ...
        try:
            cursor.reset()
        except honua_gp.HonuaGpConfigurationError:
            pass
...
def test_insert_cursor_reset_outside_context_raises_configuration_error() -> None:
    ...
    cursor = honua_gp.da.InsertCursor("roads", ["STATUS"])
    with pytest.raises(honua_gp.HonuaGpConfigurationError) as info:
        cursor.reset()
...
def test_update_cursor_update_row_length_mismatch_raises_configuration_error(
    ...
    surface as ``HonuaGpConfigurationError`` (an ``ExecuteError``
...
def test_row_length_mismatch_caught_by_execute_error(stub_clients) -> None:
    ...
    row-length error. ``HonuaGpConfigurationError`` subclasses
    ``ExecuteError`` (== top-level ``honua_gp.ExecuteError``), so this is
