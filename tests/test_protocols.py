...
def test_ogc_records_sync_pagination_helpers_and_aliases() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        query = dict(request.url.params.multi_items())
        offset = int(query.get("offset", "0"))
    ...
    with HonuaClient("http://example.test", transport=transport) as client:
        ...
        # record_pages on the top-level client delegates to the collection.
        pages = list(records_client.record_pages("catalog", page_size=2, limit=4))
        ...
        # records_all + iter_records (+ items alias) on both client and collection.
        assert [r["id"] for r in records_client.records_all("catalog", page_size=2, limit=2)] == [
            "record-1",
            "record-2",
        ]
        assert [r["id"] for r in records_client.iter_records("catalog", page_size=2, limit=2)] == [
            "record-1",
            "record-2",
        ]

        collection = records_client.collection("catalog")
        assert collection.items(limit=1)["records"][0]["id"] == "record-1"
        assert collection.item("record/9", response_format="json")["records"][0]["id"] == "record-1"
        assert [r["id"] for r in collection.items_all(page_size=2, limit=2)] == ["record-1", "record-2"]

        # openapi() + top-level items()/item() aliases + collection queryables().
        assert "records" in records_client.openapi()
        assert records_client.items("catalog", limit=1)["records"][0]["id"] == "record-1"
        assert records_client.item("catalog", "rec/1")["records"][0]["id"] == "record-1"
        assert "records" in collection.queryables()


def test_ogc_records_pagination_edge_cases() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
    ...
    with HonuaClient("http://example.test", transport=transport) as client:
        ...
        # limit=0 -> no pages (early return branch).
        assert list(collection.record_pages(limit=0)) == []
        # Single short page, no next link -> terminates after one page.
        pages = list(collection.record_pages(page_size=5, limit=5))
        assert len(pages) == 1
        assert [r["id"] for r in collection.records_all(page_size=5, limit=5)] == ["only-1"]
