...
import honua_arcpy
from honua_arcpy._process_tools import Result
...
def _fast_poll(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import honua_arcpy._process_jobs as jobs
...
def test_buffer_projects_layer_id_distance_unit_and_dissolve(_isolated_audit_dir: Path) -> None:
    ...
    result = honua_arcpy.analysis.Buffer(
...
def test_buffer_dissolve_none_maps_to_dissolve_false(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.analysis.Buffer("honua://services/t/0", "out", 10, dissolve_option="NONE")
...
def test_buffer_registers_output_alias(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.analysis.Buffer("honua://services/t/0", "roads_buffer", "5 Meters")
...
def test_buffer_writes_single_audit_line(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.analysis.Buffer("honua://services/t/0", "out", "5 Meters")
...
def test_spatial_join_projects_layer_ids_and_predicate(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.analysis.SpatialJoin(
...
def test_dissolve_projects_group_by_fields(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.management.Dissolve(
...
def test_copy_projects_source_and_target(_isolated_audit_dir: Path) -> None:
    ...
    result = honua_arcpy.management.Copy("honua://services/stage/4", "published")
...
def test_copy_features_alias_uses_same_process(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.management.CopyFeatures("honua://services/stage/0", "out")
...
def test_project_projects_layer_id_and_target_srid(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.management.Project("honua://services/roads/2", "roads_wgs84", 4326)
...
def test_project_numeric_string_srid(_isolated_audit_dir: Path) -> None:
    ...
    honua_arcpy.management.Project("honua://services/roads/0", "out", "3857")
...
def test_project_non_numeric_srid_raises(_isolated_audit_dir: Path) -> None:
    ...
    with pytest.raises(honua_arcpy.HonuaArcpyConfigurationError):
        honua_arcpy.management.Project("honua://services/roads/0", "out", "GCS_WGS_1984")
    assert proc.calls == []
...
def test_failed_job_raises_execute_error_with_message(_isolated_audit_dir: Path) -> None:
    ...
    with pytest.raises(honua_arcpy.ExecuteError) as info:
        honua_arcpy.management.Copy("honua://services/s/0", "out")
...
def test_failed_job_rolls_back_output_alias(_isolated_audit_dir: Path) -> None:
    ...
    with pytest.raises(honua_arcpy.ExecuteError):
        honua_arcpy.management.Copy("honua://services/s/0", "scratch")

    # A failed job must not leave the output alias behind (so a retry is not
    # blocked by the duplicate-output guard).
    assert honua_arcpy.get_session().get_layer("scratch") is None
...
def test_unresolvable_layer_id_raises_resolve_error(_isolated_audit_dir: Path) -> None:
    ...
    with pytest.raises(honua_arcpy.HonuaArcpyResolveError):
        honua_arcpy.management.Project("honua://services/roads/notalayer", "out", 4326)
    assert proc.calls == []
