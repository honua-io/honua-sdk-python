...
[`packages/honua-gp/LICENSE`](../../packages/honua-gp/LICENSE)) which
...
* [Compatibility matrix](compatibility-matrix.md) -- generated from the
  in-code manifest at `packages/honua-gp/honua_gp/_compat.py`.
...
* Programmatic: `honua_gp.configure(base_url=..., api_key=...)` or
  `honua_gp.configure(base_url=..., bearer_token=...)`. Pre-built
...
* Environment: `HONUA_BASE_URL`, `HONUA_API_KEY`, and `HONUA_BEARER_TOKEN`
  are picked up by `honua_gp.configure_from_env()`.
...
`HONUA_GP_PATH_MAP='{"local_name": "honua://services/foo/0"}'` for
...
-- so a `HONUA_GP_PATH_MAP` entry for `"roads"` rewrites `"roads"` whether
...
targets the same output name raises `HonuaGpConfigurationError`. Set
...
`HonuaGpConfigurationError` when none is present. A row passed to
`updateRow` / `insertRow` whose length disagrees with `field_names` raises
`HonuaGpConfigurationError` (a subclass of `ExecuteError`) with the
...
no audit signal. The refusal surfaces as `HonuaGpConfigurationError`
...
`honua_gp._resolve.descriptor_mapping`. The default heuristic is
...
`HONUA_GP_PATH_MAP` so the descriptor matches the deployment.

## Audit JSONL

Every shim call writes one JSONL line to
`${HONUA_GP_AUDIT_DIR:-./.honua-gp/audit}/audit-YYYYMMDD.jsonl` in UTC,
...
`HonuaGpUnsupportedError` with a replacement hint and tracking ticket.
...
* `honua_gp.ExecuteError` mirrors `arcpy.ExecuteError`; existing
...
* `honua_gp.HonuaGpUnsupportedError` subclasses `ExecuteError` and is
...
* `honua_gp.HonuaGpConfigurationError` is raised when the session is
...
* `honua_gp.HonuaGpResolveError` is raised when an arcpy path cannot be
  mapped to a Honua source descriptor.

## CLI

```bash
honua-gp assess path/to/inventory.json                                            # scanner-handoff pivot
honua-gp matrix --check docs/honua-gp/compatibility-matrix.md                  # CI doc-gate: fails on drift
honua-gp matrix --output docs/honua-gp/compatibility-matrix.md                 # regenerate the matrix file
```
...
ticket, not under **Supported**. It writes `honua-gp-assessment.json`
...
`packages/honua-gp/pyproject.toml` carries the `Private :: Do Not Upload`
...
design's R1 recommendation), the contents of `packages/honua-gp/` move
