...
def test_env_attributes_mirror_to_session() -> None:
    session = honua_gp.get_session()
    honua_gp.env.workspace = "honua://services/test"
    honua_gp.env.overwriteOutput = True
    honua_gp.env.outputCoordinateSystem = 4326
    honua_gp.env.parallelProcessingFactor = "75%"
    honua_gp.env.scratchWorkspace = "honua://services/scratch"
...
def test_env_unknown_attribute_falls_into_extra_env_options() -> None:
    ...
    honua_gp.env.someExperimentalFlag = "yes"
    session = honua_gp.get_session()
    assert session.extra_env_options["someExperimentalFlag"] == "yes"
    assert honua_gp.env.someExperimentalFlag == "yes"
...
def test_env_unknown_attribute_does_not_break_client_construction() -> None:
    ...
    honua_gp.configure(base_url="https://example.com", api_key="k")
    ...
    honua_gp.env.extent = "0 0 100 100"
    honua_gp.env.MTolerance = 0.001
    honua_gp.env.geographicTransformations = "WGS_1984_(ITRF00)_To_NAD_1983"
    ...
    client = honua_gp.get_session().client()
...
def test_env_reads_default_to_none_when_unset() -> None:
    assert honua_gp.env.workspace is None
    assert honua_gp.env.overwriteOutput is False
