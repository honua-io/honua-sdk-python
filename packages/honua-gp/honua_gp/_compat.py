...
* ``honua-gp assess`` (pivots scanner inventories against this table).
...
class FunctionEntry:
    ...
    :func:`honua_gp._resolve.resolve` (including ``HONUA_GP_PATH_MAP``
...
COMPAT_REPO_DOC = "docs/honua-gp/compatibility-matrix.md"
...
``HonuaGpUnsupportedError.compat_anchor`` resolves when a customer pastes
it into the repo browser. The matching package-local copy under
``packages/honua-gp/docs/`` is byte-equal -- the rendered manifest is the
...
# * ``backend="session"`` -- handled inside ``honua_gp`` itself
#   (``MakeFeatureLayer``, ``MakeTableView``).
# * ``backend="not_implemented"`` -- raises
#   :class:`~honua_gp._errors.HonuaGpUnsupportedError`. The replacement
...
COMPAT: dict[str, FunctionEntry] = {
    ...
    "management.Describe": FunctionEntry(
        ...
        notes=(
            ...
            "out of scope: the optional arcpy data_type filter argument is accepted and "
            "recorded in the audit trail but not honored -- it never filters or "
            "disambiguates, every call describes the resolved dataset as-is; .catalogPath is the shim's "
