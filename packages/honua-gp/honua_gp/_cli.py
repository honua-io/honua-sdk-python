...
* ``assess`` -- read an ``ArcPyScriptInventoryArtifact`` produced by
  ...
  hints, and writes ``honua-gp-assessment.json`` for the migration tool.
        ...
        if entry is not None:
            ...
            rows.append(
                AssessmentRow(
                    ...
                    status="out-of-scope",
                    backend="unknown",
                    notes="Not in honua-gp MVP scope (sa/na/ddd/mp).",
                    replacement_hint="Open a backlog ticket; see docs/honua-gp/scanner-handoff.md.",
                    tracking=None,
                )
            )
    return rows
...
def render_assessment(rows: Sequence[AssessmentRow]) -> str:
    ...
    out: list[str] = []
    out.append("honua-gp assessment")
...
def _assess(args: argparse.Namespace) -> int:
    ...
    machine_path = output_dir / "honua-gp-assessment.json"
...
def render_compat_matrix() -> str:
    ...
    lines.append(f"* Stubbed (raise ``HonuaGpUnsupportedError``): {stubs}")
    lines.append("")
    lines.append(
...
def _status_legend() -> str:
    return (
        ...
        "**Stub** raises ``HonuaGpUnsupportedError`` with a replacement hint and tracking ticket."
    )
    ...
    if args.check is not None:
        ...
        if existing != text:
            sys.stderr.write(
                ...
                "Re-run `honua-gp matrix --output <path>` and commit the result.\n"
            )
            return 1
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="honua-gp", description=__doc__)
    ...
    assess.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory to write honua-gp-assessment.json (default: alongside the inventory).",
    )
