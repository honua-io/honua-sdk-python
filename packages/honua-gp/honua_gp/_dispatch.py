...
LOGGER = logging.getLogger("honua_gp.dispatch")
...
def raise_unsupported(
    ...
    *,
    ...
    """Raise ``HonuaGpUnsupportedError`` and record an audit line.
...
def _project_to_process_inputs(
    ...
    *,
    ...
    for arcpy_name, value in bound.items():
        ...
        # through ``resolve()`` would let a HONUA_GP_PATH_MAP entry that
...
def _resolve_source_value(value: Any, *, session: HonuaSession) -> Any:
    ...
    resolved individually so ``HONUA_GP_PATH_MAP`` overrides apply inside
    ...
    **kwargs: Any,
    ...
    with record_call(qualified_name, args=args, kwargs=kwargs, writer=session.audit_writer()) as record:
        ...
        try:
            ...
            LOGGER.warning("honua_gp.%s failed via process %s: %s", qualified_name, entry.process_id, exc)
            raise ExecuteError(
    ...
    **kwargs: Any,
    ...
    with record_call(qualified_name, args=args, kwargs=kwargs, writer=session.audit_writer()) as record:
        try:
            ...
            kind = exc.__class__.__name__
            LOGGER.warning("honua_gp.%s failed via admin client: %s", qualified_name, exc)
            raise ExecuteError(
                f"{qualified_name} failed: {exc}",
                function=qualified_name,
                error_kind=kind,
    ...
    **kwargs: Any,
    ...
    with record_call(qualified_name, args=args, kwargs=kwargs, writer=session.audit_writer()) as record:
        try:
            ...
            kind = exc.__class__.__name__
            LOGGER.warning("honua_gp.%s failed via source client: %s", qualified_name, exc)
            raise ExecuteError(
                f"{qualified_name} failed: {exc}",
                function=qualified_name,
                error_kind=kind,
