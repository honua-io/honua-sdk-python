...
def test_configure_invalidates_lazy_client_cache_on_base_url_change() -> None:
    ...
    session = get_session()
    first = object()
    honua_arcpy.configure(base_url="https://a.example.com", api_key="k1", client=first)
    ...
    honua_arcpy.configure(base_url="https://b.example.com")
...
def test_configure_invalidates_lazy_admin_on_api_key_change() -> None:
    ...
    honua_arcpy.configure(api_key="rotated")
...
def test_configure_invalidates_lazy_processes_on_bearer_token_change() -> None:
    ...
    honua_arcpy.configure(bearer_token="rotated-bearer")
    assert session._processes is None  # noqa: SLF001
...
def test_configure_invalidates_on_extra_client_options_change() -> None:
    ...
    session = get_session()
    first = object()
    honua_arcpy.configure(base_url="https://a.example.com", client=first)
